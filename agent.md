...
4. **模板 16 种**：`free`（空白）+ cornell / meeting_5w2h / six_hats / eisenhower_matrix / monthly_plan / weekly_plan / daily_plan / woop / ride / prep_method / four_d_work / empathy_map / smart_goal / grai / trust_equation（`lib/types.ts` 的 `TemplateType` 为准）。
